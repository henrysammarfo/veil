...
export function ProofConsole({
  ...
  const { proofs, loading } = useVeilData();
  ...
  const filtered = useMemo(() => {
    ...
    return proofs.filter((p) => {
      ...
      if (q && !(p.text.toLowerCase().includes(q) || p.hash.toLowerCase().includes(q)))
        return false;
      return true;
    });
  }, [proofs, active, query, orderId]);
      ...
      {loading ? (
        ...
        <ul className="max-h-[420px] divide-y divide-[color:var(--ds-border)] overflow-auto rounded-lg border border-[color:var(--ds-border)] bg-[color:var(--ds-pill)]">
          {filtered.slice(0, max).map((p) => {
            ...
            const rowCls =
              "grid grid-cols-[auto_auto_minmax(0,1fr)_auto] items-start gap-3 px-3 py-2.5 font-mono text-[11px] leading-relaxed transition-colors hover:bg-[color:var(--ds-hover)]";
            return (
              <li key={p.id}>
                {linkEach ? (
                  <Link
