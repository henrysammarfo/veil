...
import { useVeilData, type ResourceKey } from "@/lib/dashboard/veilStore";

/**
 * Manual refresh button with a live "next update in Ns" countdown,
 * Refresh cadence driven by veil-api polling intervals + last fetch tick.
 */
export function RefreshBar({ resource, label }: { resource: ResourceKey; label?: string }) {
  const { ticks, intervals, refresh, loading } = useVeilData();
