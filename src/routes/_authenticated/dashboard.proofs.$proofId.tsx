...
function ProofDetailPage() {
  ...
  const { getProof, getOrder } = useMockData();
  ...
  if (!proof) {
    return (
      <DSEmpty
        ...
        body={`No proof with id ${proofId} in the current mock dataset. It may have rotated out of the live stream — check the Walrus archive.`}
        cta={
          <Link
            to="/dashboard/proofs"
    ...
    <div className="space-y-6">
      ...
      <DSCard>
        <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-4 sm:flex sm:flex-wrap sm:justify-between">
          <div className="min-w-0">
            <div className="flex flex-wrap items-center gap-2 font-mono text-[10px] uppercase tracking-[0.15em]">
              <span className={`rounded border px-1.5 py-0.5 ${TAG_COLOR[proof.tag]}`}>{proof.tag}</span>
        ...
        <dl className="mt-8 grid gap-3 sm:grid-cols-2">
          {fields.filter((f) => f.v).map((f) => (
            <div key={f.k} className="rounded-xl border border-[color:var(--ds-border)] bg-[color:var(--ds-pill)] p-4">
              <dt className="font-mono text-[10px] uppercase tracking-[0.18em] text-[color:var(--ds-muted)]">{f.k}</dt>
              <dd className="mt-1 flex items-center justify-between gap-2 break-all font-mono text-[12px]">
                ...
                >
                  {copied === f.k ? <Check className="h-3 w-3 text-emerald-400" /> : <Copy className="h-3 w-3" />}
                </button>
              </dd>
            </div>
          ))}
      ...
      {order && (
        <DSCard>
          ...
          >
            <div className="min-w-0">
              <div className="font-mono text-[11px] text-[color:var(--ds-muted)]">{order.id} · {order.asset}</div>
