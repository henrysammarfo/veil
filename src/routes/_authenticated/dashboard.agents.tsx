...
const ENGINES = [
  { name: "veil-bull",   state: "EXECUTING", intent: "Accumulate BTC over 7d · $50k", k: 11, filled: 7, mode: "BULL",  pcr0: "0x8c2fa91d…102", desc: "Drips buys into the order book when depth is thick and the spread is tight." },
  { name: "veil-bear",   state: "SETTLED",   intent: "Distribute ETH over 14d · $20k", k: 9,  filled: 9, mode: "BEAR",  pcr0: "0x55b10fa2…091", desc: "Range-sells with a stealth ladder, never showing the full clip." },
  { name: "veil-earn",   state: "ACCRUING",  intent: "Auto-compound USDC vault",     k: 30, filled: 18, mode: "EARN", pcr0: "0x9d4c77a1…22f8", desc: "Routes idle balance into the best risk-adjusted yield." },
  { name: "veil-router", state: "IDLE",      intent: "—",                            k: 0,  filled: 0,  mode: "UTIL", pcr0: "0x71a044ce…110", desc: "The smart-order-router every other engine calls into for fills." },
];

function EnginesPage() {
  const { addOrder } = useMockData();
  const { isPro } = useCockpitMode();
  const [open, setOpen] = useState(false);
  // touch addOrder so eslint doesn't complain
  void addOrder;
      ...
      <div className="grid grid-cols-[minmax(0,1fr)_auto] items-end gap-4 sm:flex sm:flex-wrap sm:justify-between">
        <div className="min-w-0">
          ...
          <p className="mt-2 max-w-xl text-sm text-[color:var(--ds-muted)]">
            ...
            engine is attested — its PCR0 hash is verifiable on Sui.
          </p>
        </div>
        <button
        ...
        {ENGINES.map((a) => (
          <DSCard key={a.name}>
            <DSSectionTitle
              ...
              action={
                <span className="flex items-center gap-1.5 font-mono text-[10px] uppercase tracking-[0.18em]">
                  <CircleDot
                    className={`h-3 w-3 ${
                      a.state === "EXECUTING" ? "animate-pulse text-emerald-400" :
                      a.state === "IDLE"      ? "text-[color:var(--ds-muted)]" :
                                                "text-amber-400"
                    }`}
                  />
                  {a.state}
            ...
            <div className="mt-4 flex items-center justify-between font-mono text-[11px] text-[color:var(--ds-muted)]">
              <span>{a.mode}</span>
              <span>{a.filled} / {a.k} slices</span>
            </div>
            <div className="mt-2 h-1 w-full overflow-hidden rounded-full bg-[color:var(--ds-pill)]">
              <div className="h-full bg-gradient-to-r from-amber-500 to-amber-300" style={{ width: `${a.k ? (a.filled / a.k) * 100 : 0}%` }} />
            </div>

            {isPro && (
              <div className="mt-4 grid grid-cols-2 gap-3 border-t border-[color:var(--ds-border)] pt-4 font-mono text-[11px]">
                <div className="flex items-center gap-2 text-[color:var(--ds-muted)]"><Cpu className="h-3 w-3" /> Enclave</div>
                <div className="truncate">{a.name}-v1.3</div>
                <div className="flex items-center gap-2 text-[color:var(--ds-muted)]"><ShieldCheck className="h-3 w-3" /> PCR0</div>
